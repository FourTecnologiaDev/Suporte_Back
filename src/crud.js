...
async function crud(tabela, registro, operacao) {
  ...
  var id = "";
  if (registro) {
    if (registro.codigo != undefined) var codigo = parseInt(registro.codigo);
    if (registro._id != undefined) id = registro._id;
  } else {
    var codigo = 1;
  }
  ...
  if (operacao == "delete") {
    if (registro.historico) {
      modelo
        .findByIdAndUpdate(id, { $pull: { historico: registro.historico[0] } })
        .exec();
    ...
    if (registro.historico) {
      resultado = modelo
        .findByIdAndUpdate(id, {
          $addToSet: { historico: registro.historico[0] },
        })
        .exec();
      delete registro.historico;
      modelo.findOneAndUpdate(filter, registro).exec();
